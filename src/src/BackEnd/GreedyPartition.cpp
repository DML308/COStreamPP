...
void GreedyPartition::SssgPartition(SchedulerSSG *sssg)
{
    if (this->mnparts == 1)
    {
        for (int i = 0; i < nvtxs; i++)
            FlatNode2PartitionNum.insert(make_pair(sssg->flatNodes[i], 0)); //建立节点到划分编号的映射

        for (int i = 0; i < nvtxs; i++)
            PartitonNum2FlatNode.insert(make_pair(0, sssg->flatNodes[i]));
    }

    nvtxs = sssg->flatNodes.size();       //顶点的个数
    int k = getParts();                   //划分的子图个数
    ...
    for (int i = 0; i < nvtxs; i++)
    {
        FlatNode2PartitionNum.insert(make_pair(sssg->flatNodes[i], part[i])); //建立节点到划分编号的映射
    }
    for (int i = 0; i < nvtxs; i++)
        PartitonNum2FlatNode.insert(make_pair(part[i], sssg->flatNodes[i]));
}

void GreedyPartition::doPartition(SchedulerSSG *sssg, int k)
{
    int edgenum = sssg->mapEdge2DownFlatNode.size(); //图的边数
    ...
    { //构造子图X[1] ~ X[k-1]
        ...
        { //一定要加X[0].size() != 0，否则137行可能会出bug
            ...
            { //如果候选集合为空
                ...
                { //choose = rand() % (X[0].size());//从X[0]中随机选择一个节点
...
int GreedyPartition::chooseMaxGain(SchedulerSSG *sssg, vector<FlatNode *> &S, vector<FlatNode *> &Xi, vector<FlatNode *> &X0)
{
    ...
    { //遍历所有候选节点
        ...
        {                                         //遍历所有输入边
            ...
            if (find(Xi.begin(), Xi.end(), p) != Xi.end())
            { //如果上端节点在Xi中
                wi += (S[i]->inPopWeights[j]) * (sssg->GetSteadyCount(S[i]));
            }
            else if (find(X0.begin(), X0.end(), p) != X0.end())
            { //如果上端节点在X0中
...
void GreedyPartition::updateCandidate(SchedulerSSG *sssg, vector<FlatNode *> &X0, vector<FlatNode *> &S, int index)
{
    ...
    {                                             //遍历所有输入边
        ...
        if (find(X0.begin(), X0.end(), p) != X0.end() && find(S.begin(), S.end(), p) == S.end())
        {                   //如果上端节点在X0中并且不在S中
...
int GreedyPartition::getTotalEdge(SchedulerSSG *sssg, int k)
{ //得到各核间通信量的大小和总大小
    edge.clear();
    edge.resize(k, 0);
    int total_edge = 0;
...
void GreedyPartition::errorDecrease(SchedulerSSG *sssg, int k)
{ //误差下降算法
    ...
    { //大于平衡因子时进行误差下降算法
        ...
        for (int i = 0; i < X[imax].size(); i++)
        {
            ...
            {                                               //遍历所有输入边
                ...
                if (partNum != imax)
                { //如果上端节点对应的子图不是imax
                    if (w[partNum] + vwgt[index] < w[imax])
                    {                                                                  //如果移动后最大子图工作量减小
...
void GreedyPartition::errorDecrease2(SchedulerSSG *sssg, int k)
{ //误差下降算法
    double we = sssg->total_work / k;
    int imax = 0;
    int imin = 0;
...
{                                           //禁忌搜索优化算法
    ...
    while (aloneVec.size() > 0)
    {
        ...
        if (cnt == aloneVec.size())
            break;
    }
}

int GreedyPartition::orderPartitionResult()
{                       //按子图负载由大到小排序,选择排序算法
